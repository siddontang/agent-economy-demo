...
def phase_2_pay_and_remember(agent):
    ...
    for cycle in range(1, cycles + 1):
        ...
        for token in tokens:
            if agent.wallet.is_live:
                print(f"  {DIM}GET /x402/simple/price?ids={token} → 402 Payment Required{RESET}")
            else:
...
def main():
    ...
    if wallet.is_live:
        print(f"{GREEN}🔑 Real x402 wallet loaded: {wallet.wallet_address[:10]}...{RESET}")
        print(f"     Using official Coinbase x402 SDK + CoinGecko x402 endpoints")
    else:
        print(f"{YELLOW}🔧 Simulation mode (no private key — use --private-key for live x402){RESET}")
        if not wallet._has_sdk and args.private_key:
            print(f"     {RED}⚠️  x402 SDK not installed. Run: pip install \"x402[requests,evm]\"{RESET}")
    print()
